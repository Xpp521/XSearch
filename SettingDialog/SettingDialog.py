...
class SettingDialog(QDialog):
    default_setting = {
        ...
        'SearchEngine/data/0': 'baidu||baidu.png||https://www.baidu.com/s?ie=utf-8&wd=%s',
        'SearchEngine/data/1': 'google||google.png||https://www.google.com/search?q=%s',
        'SearchEngine/data/2': 'bing||bing.png||http://www.bing.com/search?q=%s',
        'SearchEngine/data/3': 'bingcn||bing.png||http://cn.bing.com/search?q=%s',
        'SearchEngine/data/4': '360||360.png||https://www.so.com/s?ie=utf-8&q=%s',
        'SearchEngine/data/5': 'sogou||sogou.png||https://www.sogou.com/web?ie=utf8&query=%s',
        'SearchEngine/data/6': 'toutiao||toutiao.png||https://m.toutiao.com/search/?&keyword=%s',
        'SearchEngine/data/7': 'yandex||yandex.png||https://yandex.com/search/?text=%s',
        'SearchEngine/default_engine': 'baidu||baidu.png||https://www.baidu.com/s?ie=utf-8&wd=%s',
        'SuggestionState': '1',
        'SuggestionProvider/index': '0',
        'SuggestionProvider/data': '360',
        'BrowserPath': '',
        'PrivateMode': '0',
    ...
    def __change_startup_state(self, checked):
        if 'win' == platform[:3]:
    ...
    def __change_suggestion_state(self, checked):
        self._ui.comboBox_suggest_engine.setEnabled(checked)
        self._setting.setValue('SuggestionState', str(int(checked)))

    def __change_suggestion_provider(self, index):
        self._setting.setValue('SuggestionProvider/index', str(index))
        self._setting.setValue('SuggestionProvider/data', self._ui.comboBox_suggest_engine.currentData())

    def _clear_suggestion_cache(self):
        pass

    def __change_browser_path(self):
    ...
    def __change_private_mode_state(self, checked):
    ...
    def __connect_event_handlers(self):
        ...
        self._ui.checkBox_start_up.clicked.connect(self.__change_startup_state)
        self._ui.checkBox_show_tip.clicked.connect(self.__change_tip_state)
        self._ui.comboBox_language.currentIndexChanged.connect(self.__change_language)

        self._ui.checkBox_search_suggestion.clicked.connect(self.__change_suggestion_state)
        self._ui.comboBox_suggest_engine.currentIndexChanged.connect(self.__change_suggestion_provider)
        self._ui.pushButton_suggestion_clear_cache.clicked.connect(self._clear_suggestion_cache)
        self._ui.pushButton_browser_path.clicked.connect(self.__change_browser_path)
        self._ui.checkBox_private_mode.clicked.connect(self.__change_private_mode_state)
        ...
        self._ui.comboBox_repetitions.currentIndexChanged.connect(self.__change_hotkey_repetitions)
        self._ui.comboBox_interval.currentIndexChanged.connect(self.__change_hotkey_interval)

        self._ui.comboBox_opacity.currentIndexChanged.connect(self.__change_opacity)
        self._ui.comboBox_fillet.currentIndexChanged.connect(self.__change_fillet)
        self._ui.comboBox_distance.currentIndexChanged.connect(self.__change_distance)
        for button in self.__theme_buttons:
            button.clicked.connect(self.__change_theme)
        self._ui.pushButton_custom.clicked.connect(lambda: None)

        self._ui.checkBox_no_sleep.clicked.connect(self.__change_no_sleep_state)
    ...
    def _load_settings(self):
        ...
        self._ui.comboBox_suggest_engine.setEnabled(suggestion_state)
        self._ui.comboBox_suggest_engine.setCurrentIndex(int(self._setting.value('SuggestionProvider/index')))
    ...
    def reload_ui(self, text=True, qss=True):
        ...
        if qss:
            ...
            self._ui.widget_right.setStyleSheet('''.QWidget {
            ...
            .QWidget QCheckBox::indicator:unchecked, .QWidget QRadioButton::indicator:unchecked {
            border-radius: {border_radius_s}px;
            border: 1px solid {theme_color};
            }'''.replace('{theme_color}',
                         theme_color).replace('{border_radius}',
                                              border_radius).replace('{border_radius_s}',
                                                                     str(int(border_radius) * 0.8)))
            self._ui.label_logo.setStyleSheet('.QLabel {color: white;}')
